...
def experimento_1(schedule_next: bool = False):
    ...
    Ordem de execução:
        ...
        - realiza o teste ndt
        - realiza teste traceroute para domínio ndt
    ...
    for i in range(num_pages):
        ...
        browser_experiments = get_browser_experiment_func('navigation')
        for method, run_func in zip(['puppeteer'], browser_experiments):
            result['browser'][method] = run_func(url, mac, SERVER)

    # reproduction experiments
    browser_experiments = get_browser_experiment_func('reproduction')
    for method, run_func in zip(['selenium'], browser_experiments):
            result['browser'][method] = run_func(url, mac, SERVER)
